...
The policy has an explicit start date. Invoice and receipt preparation uses the document's actual issue timestamp; reminders do not run before the configured start date. Turning automation on is not an unrestricted historical mailing operation. Repeated preparation and polling use durable logical-event identities and do not create duplicate invoice/receipt emails.
