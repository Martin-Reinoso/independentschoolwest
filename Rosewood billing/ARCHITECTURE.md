...
Live Xero OAuth/webhook sync, gateway collection, automated bank ingestion, parent login, split-liability rules, annual Rosewood policy automation, historical migration and multi-school tenancy are subsequent projects. Their seams are documented; this build must not offer misleading enabled buttons for them. Actual production deployment is separate from committing/building/testing: it requires approved business settings, access identities, hosting, reconciliation owner, backup destination and tax mappings. Nothing about the independent build changes the currently published enrolment form.

## Documents and communications extension — 12 September 2026

Following the initial validated release, a document centre, staff account administration and a durable communications outbox were added. The PDF renderer remains the authority for printed values. Communications freezes the document projection using a synchronous ledger snapshot transaction and rechecks current permission/recipient/financial facts before dispatch. Its additive `comms_*` tables own messages, settings, holds, attempts, dispatch markers, outcomes, manual resolutions and immutable events. `auth_*` remains responsible for staff lifecycle and sessions.

The web server prepares and approves work but performs no email network calls. An optional separately operated SES worker renders attachments and sends outside SQLite transactions. Default delivery is disabled and demo delivery is impossible. Review/automatic policies, durable event deduplication, pending-payment reminder suppression, evidence-backed uncertain-outcome recovery and resends are documented in [Communications](COMMUNICATIONS.md). Provider acceptance is explicitly distinct from actual delivery; bounce/read tracking is not implemented. This extension does not change the enrolment or Xero integration boundary.
