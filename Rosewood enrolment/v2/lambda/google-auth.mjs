...
export class GoogleAccessTokenProvider {
  constructor({
    ...
    if (requestedMode && !["service_account", "user_oauth"].includes(requestedMode)) {
      throw new Error("GOOGLE_AUTH_MODE must be service_account or user_oauth.");
    }
    if (!requestedMode && serviceComplete && oauthComplete) {
      throw new Error("Set GOOGLE_AUTH_MODE when both Google credential sets are present.");
    }
