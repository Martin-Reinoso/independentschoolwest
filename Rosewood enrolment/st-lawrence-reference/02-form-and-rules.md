...
### ACC-09 To ACC-14 Remote Signing Observations
...
- Review explains only near its bottom that comments can be added in Sign and requires a
  separate reviewed-and-ready confirmation before Next. Its disabled `Pending Signature`
  preview is easily mistaken for the actual signing interface.
- Checking the Review confirmation changes Next from grey to green. The dedicated Sign
  page then provides optional comments, two required declarations, a locked signature
  canvas, automatic date and gated Next.
- Unchecking declarations after interaction produces separate IP-address and terms errors,
  a red signature border with `Please provide your signature`, and a red date outline.
- The user completed the live signature. Thank You confirmed Signing Complete, and View
  Signed Form opened a read-only Submitted agreement with server Last Updated,
  Print/Logout and both guardian signatures complete.
...
| APP-03 | Required guardian signature is incomplete | Form status remains Pending Signatures and the additional guardian receives a signed Contact Portal task link | Yes for completed submission, based on observed state and confirmation emails | Required guardian confirms the prefilled email, passes invisible Turnstile, verifies with a 30-minute OTP, confirms prefilled details, reviews the complete read-only form, completes the electronic signature and receives an individual acknowledgement | SLB-003, SLB-017 to SLB-021, SLB-EMAIL-015 to SLB-EMAIL-019 |
| ACC-11 | Remote guardian has not confirmed Review | `I have reviewed the form and am ready to proceed` is unchecked and Next remains grey/disabled | Yes | Check the confirmation; Next turns green and opens the dedicated Sign stage | SLB-019, SLB-020 |
| ACC-12 | Required signing declarations are incomplete | Signature canvas says `Please agree to the terms above to enable signing`; unchecking produces declaration-specific errors, red canvas/date validation and `Please provide your signature`; Clear Signature and Next are disabled | Yes | Accept both declarations, provide the signature and allow the date to populate | SLB-020, SLB-021 |
...
| SLB-RULE-021 | The active guardian signs on one local canvas. A second contactable guardian removes the one-signature explanation but does not add a second local canvas. | A dedicated signed Contact Portal task, prefilled identity check, invisible Turnstile, 30-minute OTP, details confirmation, full read-only review, dedicated Sign stage, Thank You and submitted view directly confirm staged remote signing rather than simultaneous local signatures. | SLB-003, SLB-006, SLB-017 to SLB-021, SLB-EMAIL-014 to SLB-EMAIL-019 |
...
| SLB-RULE-028 | The remote guardian signs only on a dedicated stage after Review confirmation; the pending signature block shown inside Review is disabled. | Review confirmation changes Next from grey to green. Sign gates its canvas behind two declarations and uses declaration-specific, canvas and date validation before enabling final Next. | SLB-020, SLB-021 |
| SLB-RULE-029 | Completing the additional guardian signature produces a five-step Thank You state and a View Signed Form action. | The guardian receives explicit completion and logout guidance before opening the durable record. | SLB-021 |
| SLB-RULE-030 | The agreement becomes Submitted only after the required signature set is complete. | View Signed Form shows a server Last Updated timestamp, both signatures complete, disabled controls and only Print/Logout actions. | SLB-003, SLB-021, SLB-EMAIL-018 |
