...
| 25E | Prepare additional acceptance signature | Additional guardian | Review confirmation succeeds and Next is selected | Dedicated Sign stage provides optional comments, two declarations, a locked signature canvas, automatic date, validation errors and gated Next | SLB-020, SLB-021 | Interface and user-triggered validation frames captured |
| 25F | Complete additional acceptance signature | Additional guardian and Enquiry Tracker | Declarations and signature are completed | Thank You confirms the individual signature; an individual-signature email is followed one second later by a separate all-signatures-complete email stating that the application is submitted for school processing | SLB-021, SLB-EMAIL-020, SLB-EMAIL-021 | Current agreement completion and both automatic messages captured |
| 25G | View completed agreement | Additional guardian | View Signed Form is selected | Immutable Submitted agreement with server timestamp, Print/Logout and both guardian signatures complete | SLB-021 | Captured read-only |
