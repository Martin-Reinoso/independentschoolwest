// Usage: NODE_PATH=<playwright node_modules> node tests/email09-v5.test.cjs [base URL]
...
const source = fs.readFileSync(path.join(root,'emails/email09-v5.html'),'utf8');
...
(async()=>{
  ...
  try {
    ...
    for(const width of [1280,390,320]){
      ...
      assert((await page.locator('#application-count').innerText()).includes('Families had completed 16 Applications for Enrolment by 6 September. Thank you for taking this exciting step with us.'));
      ...
      assert(links.some(h=>h.endsWith('/presentation/')));
      assert((await page.locator('h1').innerText()).replace(/\s+/g,' ').includes('Applications for Enrolment are now open'));
      assert((await page.locator('.mobile-button').first().getAttribute('href')).endsWith('/presentation/'));
      assert.equal(await page.getByText('Already completed an application?',{exact:true}).count(),0);
      assert.deepEqual(errors,[]);
      await page.screenshot({path:`/tmp/email09-v5-final-${width}.png`,fullPage:true});
